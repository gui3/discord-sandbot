...
// !!! A AJOUTER PLUS TARD !!!
// le tableau est donné pour un nombre fini de distances
// pour une distance intermédiaire (par ex 17 mètres),
// faire la moyenne entre les deux distances encadrantes (15 et 20)
...
//    var [x, y] = arguments;  // méthode non robuste
    // méthode alternative: ignore tout ce qui est écrit après les deux premiers nombres
    var arg_join = arguments.join(" ");
    var [dist, modif] = arg_join.split(" ",2);

    // vérifier que l'on a deux nombres
    if (parseInt(dist) && parseInt(modif)) {
      // tirage du d100
      ...
      // analyse du résultat (échec ou réussite)
      if (test_tir >= dd_table[ind]) {
        var reussite = test_tir - dd_table[ind];
        reply += " (dépassé de " + reussite + ")\n";
        // division euclidienne pour obtenir le chiffre des dizaines
